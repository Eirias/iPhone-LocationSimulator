...
public final class GoIOS: @unchecked Sendable {
    ...
    static func resolveBinary() throws -> URL {
        // Prefer the binary bundled with the app, so it's self-contained (no separate
        // `go-ios` install required).
        if let bundled = try? extractedBundledBinary() {
            return bundled
        }
        // Fall back to a system / developer install.
        let candidates = candidatePaths()
        for path in candidates where FileManager.default.isExecutableFile(atPath: path) {
            return URL(fileURLWithPath: path)
        }
        throw GoIOSError.binaryNotFound(searched: candidates + ["(bundled go-ios)"])
    }

    /// The go-ios binary shipped inside the app via SwiftPM resources. Bundle resources are
    /// read-only and may lack the executable bit, so we extract it once to Application
    /// Support and `chmod +x`, re-copying only when the bundled binary changes (app update).
    /// Returns nil if no binary is bundled (e.g. SwiftPM-only/test contexts without it).
    private static func extractedBundledBinary() throws -> URL? {
        guard let bundled = Bundle.module.url(forResource: "go-ios", withExtension: nil as String?) else {
            return nil
        }
        let fm = FileManager.default
        let dir = try fm.url(for: .applicationSupportDirectory, in: .userDomainMask,
                             appropriateFor: nil, create: true)
            .appendingPathComponent("LocationSimulator/bin", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        let dest = dir.appendingPathComponent("go-ios")

        // Re-extract only when the bundled binary changed (app update) or dest is missing.
        ...
        if !fm.isExecutableFile(atPath: dest.path) || bundledSize != destSize {
